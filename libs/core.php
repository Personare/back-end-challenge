...
class API {
    ...
    protected function listCommandInternal($parameters) {
        ...
        if ($result = $this->db->query($sql,$params)) {
            ...
            echo '"data":[';
            $first_row = true;
            while ($row = $this->fetchAssoc($result,$fields[$table])) {
                if ($first_row) $first_row = false;
                else echo ',';
                if (isset($collect[$table])) {
                    foreach (array_keys($collect[$table]) as $field) {
                        $collect[$table][$field][$keys[$field]] = $row[$keys[$field]];
                    }
                }
    ...
    protected function listCommand($parameters) {
        ...
        if ($transform) {
            ...
            echo json_encode(self::transform($data));
        }
        return false;
    }
    ...
    public static function transform(&$tables) {
