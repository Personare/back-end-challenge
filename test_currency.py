...
class TestCurrency(unittest.TestCase):
    ...
    def test_string_decimal_input_eur_outcome(self):
        brl = '1,20'
        expected = self.eur_unit * self.curr.floatfy(brl)
    ...
    def test_string_decimal_input_usd_outcome(self):
        brl = '1,20'
        expected = self.usd_unit * self.curr.floatfy(brl)
    ...
    def test_eur_input(self):
        eur = 1.0
        expected = self.curr.floatfy(eur) / self.eur_unit
        brl = self.curr.fromEUR(eur)
        self.assertEqual(expected, brl)

    def test_usd_input(self):
        usd = 1.0
        expected = self.curr.floatfy(usd) / self.usd_unit
